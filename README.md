...
- **Top row (A–D).** Portal venous phase T1-weighted imaging in a cirrhotic patient with left lobe elongation (external test set).
- **Middle row (E–H).** T2-weighted imaging in a cirrhotic patient with severe ascites (external test set).
- **Bottom row (I–L).** Diffusion-weighted imaging in a patient with a large intrahepatic cholangiocarcinoma extending to the liver margins (internal test set).
